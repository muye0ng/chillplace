...
BEGIN
    ...
    -- 2. NextAuth 관련 데이터 삭제 (CASCADE로 자동 삭제되지만 명시적으로)
    DELETE FROM public.sessions WHERE user_id = user_id_input;
    GET DIAGNOSTICS deleted_records = ROW_COUNT;
    RAISE NOTICE '🗑️ sessions: % 건 삭제', deleted_records;
    
    DELETE FROM public.accounts WHERE user_id = user_id_input;
    GET DIAGNOSTICS deleted_records = ROW_COUNT;
...
-- 테스트 (주석 해제해서 사용):
-- SELECT delete_user_by_email('test@example.com');

SELECT '✅ 완전한 회원 삭제 시스템 설정 완료!' as result; 